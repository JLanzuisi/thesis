...
\begin{proof}[Demostración del \cref{teo:kunen}]
	...
	$\ran(S)$ es un partición de $\set{\xi < \lambda^+\mid \cf(\xi) = \omega}$.

	Puesto que $j(\lambda)=\lambda$, se tiene $\lambda^+\leq j(\lambda^+) = {\lambda^+}^M\leq \lambda^+$
	y entonces prevalece la igualdad. Por la elementalidad de $j$, $j(S)\colon j(\kappa)\to\pwset{\lambda^+}$ y
	\[
		(j(S)\subseteq\set{\xi < \lambda^+\mid \cf(\xi) = \omega}\,\text{es estacionario en $\lambda^+$})^M.
	\]
\end{proof}
