...
porque su estructura se asemeja al teorema
...
con ZFC. Ambos teoremas tratan de un implicación directa, a diferencia de los resultados
típicos de la teoría de cardinales grandes que, normalmente, establecen resultados
de consistencia relativa.
...
\begin{defi}
    ...
    \]
    Si $f$ es la función identidad, diremos que $\model{A}$
...
el concepto de inmersión elemental para clases propias, pues esto requiere de teorías
como la de Morse-Kelley. Dicho esto, lo que sí se puede formalizar en ZFC \note{citation needed}
...
entre modelos internos es también una $\Sigma_n$-inmersión. Teniendo todo esto en cuenta,
a partir de ahora por inmersión elemental se referirá a $\Sigma_1$-inmersiones entre clases propias
transitivas estructuradas por $\in$.

Dada una inmersión elemental $j\colon A\to B$, un resultado sencillo de demostrar
dice que $j(\alpha)>\alpha$ siempre que $\alpha$ sea un ordinal de $A$. La siguiented
definición se sigue de este hecho.

\begin{defi}
    Sea $j\colon A\to B$ una inmersión elemental. El \concept{punto crítico} de $j$
