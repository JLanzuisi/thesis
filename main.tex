...
\begin{proof}
    ...
    la elementaridad de $j$ junto con $j(E)=E$ y el hecho de que
...
dada por los siguiente dos teoremas. Primero, una condición extra sobre $U$ da como resultado
modelos bien fundados y, además, la relación $\in_U$ es tipo-conjunto.
...
\begin{proof}
    ...
    \[
        ...
        \in U,
    \]
    para $k\in\omega$ y la sucesión $\seq{(g_n)_U^0\mid n\in\omega}$ es testigo de que $\in_U$
    no esta bien fundada.
\end{proof}

\begin{teo}\label{teo:Eu-set-like}
    La relación $\in_U$ es tipo-conjunto.
\end{teo}
\begin{proof}
    \def\Ou{^0_U}
    Sean $(g)\Ou, (f)\Ou\in \Ult(U,V)$ tales que $(g)\Ou \in_U (f)\Ou$ y $g_0\in(g)\Ou$.
    ...
    y se tiene que $\set{(g)\Ou\mid (g)\Ou\in_U (f)\Ou}\subseteq V_{\rank(f)+2}$ es un conjunto.
\end{proof}
...
\]
además, debido al teorema fundamental de los ultraproductos, $M_U$ es un modelo interno
de ZFC.

A partir de ahora, se usará la notación $[f]_U$ para referirse a $\pi_U((f)_U^0)$
con $f\colon S\to V$. En algunos casos, el ultrafiltro $U$ será claro dado el contexto,
y se utilizará la notación $[f]$. Sea $f_x$ la función constantemente igual a $x$ y,
...
\[
    j_U(x) = [f_x]_U.
\]
La función $j_U$ es una inmersión de $V$ en $M_U$, debido al teorema fundamental.
